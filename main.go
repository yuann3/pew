...
const pewMarker = "<!-- Generated by Pew -->"
...
func main() {
	...
	// Determine the final output file, handling overwrite prompt if needed
	outputFile := f.outputFile
	if _, err := os.Stat(outputFile); err == nil {
		outputFile, err = promptForOutputFile(outputFile)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error handling output file: %v\n", err)
			os.Exit(1)
		}
	}

	if f.dumpDir != "" {
		markdown, err = processDirectory(f.dumpDir, f.noDefaultIgnores, outputFile)
		if err != nil {
	...
	// Write to file
	err = writeMarkdownFile(outputFile, markdown)
	if err != nil {
...
// Prompts user to overwrite or specify a new filename if the output file exists
func promptForOutputFile(outputFile string) (string, error) {
	fmt.Printf("Output file '%s' already exists.\n", outputFile)
	fmt.Println("Do you want to overwrite it? (y/yes/Enter for overwrite, n/no for new filename)")
	...
	if response == "" || response == "y" || response == "yes" {
		// Explicitly remove the existing file to ensure overwrite
		if err := os.Remove(outputFile); err != nil {
			return "", fmt.Errorf("failed to delete existing file: %w", err)
		}
		return outputFile, nil
	} else if response == "n" || response == "no" {
		fmt.Println("Please enter a new filename:")
		newFile, err := reader.ReadString('\n')
		if err != nil {
		...
		return "", fmt.Errorf("invalid input: please enter 'y', 'yes', 'n', 'no', or press Enter")
	}
}
...
func processDirectory(dumpDir string, noDefaultIgnores bool, outputFile string) (string, error) {
	...
	fileList, err := collectTextFiles(absDir, ignorePatterns, outputFile)
	if err != nil {
		return "", fmt.Errorf("failed to collect files: %w", err)
	}

	tree, err := generateTree(absDir, ignorePatterns, outputFile)
	if err != nil {
...
func isPathIgnored(path, rootDir string, patterns []string, outputFile string) bool {
	if len(patterns) == 0 && outputFile == "" {
		return false
	}
	...
	// Check if the path matches the output file
	if outputFile != "" {
		...
		// Also check if the relative path matches the output file's base name
		if filepath.Base(outputFile) == filepath.Base(path) {
			return true
		}
	}

	// Check if the file is a pew-generated Markdown file
	if strings.ToLower(filepath.Ext(path)) == ".md" {
		if isPewGenerated(path) {
			return true
		}
	}
...
// Checks if a Markdown file was generated by pew by looking for the marker
func isPewGenerated(path string) bool {
	file, err := os.Open(path)
	if err != nil {
		return false // If we can't read the file, assume it's not pew-generated
	}
	defer file.Close()

	scanner := bufio.NewScanner(file)
	// Read up to the first 5 lines to find the marker
...
func collectTextFiles(rootDir string, ignorePatterns []string, outputFile string) ([]string, error) {
	var fileList []string
	...
	err := filepath.Walk(rootDir, func(path string, info fs.FileInfo, err error) error {
		...
		if isPathIgnored(path, rootDir, ignorePatterns, outputFile) {
			if info.IsDir() {
...
func generateTree(rootDir string, ignorePatterns []string, outputFile string) (string, error) {
	var buf bytes.Buffer
	buf.WriteString(filepath.Base(rootDir) + "/\n")

	err := printDir(rootDir, "", &buf, rootDir, ignorePatterns, outputFile)
	if err != nil {
...
func printDir(dir, prefix string, buf *bytes.Buffer, rootDir string, ignorePatterns []string, outputFile string) error {
	...
	for i, file := range visibleFiles {
		...
		if file.IsDir() {
			buf.WriteString("/\n")
			if err := printDir(path, newPrefix, buf, rootDir, ignorePatterns, outputFile); err != nil {
				return err
			}
		} else {
			buf.WriteString("\n")
